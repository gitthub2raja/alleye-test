        ...
        <div ref={dashboardRef} className="space-y-6 bg-background p-1">
             <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p className="text-text-secondary text-sm">Deep dive into learner behavior across all platforms.</p>
                </div>
                <div className="flex items-center gap-2">
                    <select value={timeFilter} onChange={e => setTimeFilter(e.target.value)} className="form-select text-sm">
                        <option value="30">Last 30 Days</option>
                        <option value="7">Last 7 Days</option>
                        <option value="90">Last 90 Days</option>
                    </select>
                     <select value={userFilter} onChange={e => setUserFilter(e.target.value)} className="form-select text-sm">
                        <option value="all">All Users</option>
                        {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                    </select>
                     <select value={contentFilter} onChange={e => setContentFilter(e.target.value)} className="form-select text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="text-center">
                    <p className="text-text-secondary">Total Statements</p>
                    <p className="text-4xl font-bold text-text-main">{stats.totalStatements}</p>
                </Card>
                 <Card className="text-center">
                    <p className="text-text-secondary">Active Learners</p>
                    <p className="text-4xl font-bold text-text-main">{stats.activeLearners}</p>
                </Card>
                 <Card className="text-center">
                    <p className="text-text-secondary">Average Score</p>
                    <p className="text-4xl font-bold text-text-main">{stats.averageScore.toFixed(2)}%</p>
                </Card>
            </div>
                ...
                <Card className="lg:col-span-3">
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                        <LineChart data={engagementData}>
                            ...
                            <Tooltip contentStyle={{ backgroundColor: 'var(--card-color)', border: '1px solid var(--border-color)'}}/>
                            <Legend />
                    ...
                    <h3 className="font-semibold mb-4">Quiz Score Distribution</h3>
                     <ResponsiveContainer width="100%" height={250}>
                        <BarChart data={scoreDistributionData}>
                           <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)"/>
                           <XAxis dataKey="name" tick={{ fill: 'var(--text-secondary-color)', fontSize: 12 }}/>
                           <YAxis tick={{ fill: 'var(--text-secondary-color)', fontSize: 12 }}/>
                           <Tooltip contentStyle={{ backgroundColor: 'var(--card-color)', border: '1px solid var(--border-color)'}}/>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <ul className="mt-2 space-y-1 list-disc list-inside">
                               {contentHotspots.mostCompleted.map(item => ( <li key={item.title}> {item.title} <span className="text-text-secondary">({item.rate.toFixed(0)}%)</span> </li> ))}
                            </ul>
                        </div>
                         <div>
                            <h4 className="font-bold text-red-400">Least Completed</h4>
                             <ul className="mt-2 space-y-1 list-disc list-inside">
                               {contentHotspots.leastCompleted.map(item => ( <li key={item.title}> {item.title} <span className="text-text-secondary">({item.rate.toFixed(0)}%)</span> </li> ))}
                            </ul>
                        </div>
                    </div>
                </Card>
